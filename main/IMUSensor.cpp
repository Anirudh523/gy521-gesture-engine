...
wake_up();
configure_ranges();
ESP_LOGI(TAG, "MUP-6050 initialized at 0x%02X", ADDR);
}
...
void IMUSensor::wake_up() {
write_register(REG_PWR_MGMT, 0x00);
vTaskDelay(pdMS_TO_TICKS(10));
}

void IMUSensor::configure_ranges(){
write_register(REG_ACCEL_CFG, 0x08);
write_register(REG_GYRO_CFG, 0x08);
}

IMUFrame IMUSensor::read() {
uint8_t buf[12];
read_registers(REG_ACCEL_OUT, buf, 6);
read_registers(REG_GYRO_OUT, buf + 6, 6);
IMUFrame iframe;
...
ESP_ERROR_CHECK(i2c_master_write_to_device(PORT, ADDR, data, sizeof(data), pdMS_TO_TICKS(10)));
}

void IMUSensor::read_registers(uint8_t reg, uint8_t* buf, size_t len) {
ESP_ERROR_CHECK(i2c_master_write_read_device(
PORT, ADDR, &reg, 1, buf, len, pdMS_TO_TICKS(10)));
}
